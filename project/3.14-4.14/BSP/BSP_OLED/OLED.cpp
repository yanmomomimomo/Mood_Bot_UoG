    ...
    for (int y = 0; y < OLED_HEIGHT; y++) {
        for (int x_byte = 0; x_byte < OLED_WIDTH / 8; x_byte++) {
            ...
            for (int bit = 0; bit < 8; bit++) {
                int x = x_byte * 8 + (7 - bit);  // 最高位在左边
                if (byte & (1 << bit)) {
                    OLED_setPixel(x, y, 1);  // 设置像素为白色
                }
            }
        }
    }
...
bool OLED_class::Word_Show(){
    ...
    DrawChar(5, 36, '1', &Font16);
    DrawChar(17, 36, '1', &Font16);
    DrawChar(31, 36, '8', &Font16);
    DrawChar(49, 36, 'C', &Font16);
    DrawChar(70, 36, '4', &Font16);
    DrawChar(82, 36, '2', &Font16);

    if (!OLED_updateDisplay()) {
        std::cerr << "更新显示失败" << std::endl;
        close(i2c_fd);
        return true;
    }
    return true;
